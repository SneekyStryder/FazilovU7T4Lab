...
public class ArrayListAlgorithms
{
    ...
    public static boolean containsTarget(ArrayList<String> stringList, String target) {
        for (String string : stringList) {
            if (string.indexOf(target) > 0 || string.equals(target)) {
                return true;
            }
        }
        return false;
    ...
    public static void wackyVowels(ArrayList<String> wordList) {
        for (int i = 0; i < wordList.size(); i++) {
            ...
            boolean containsVowel = (word.indexOf("a") != -1 || word.indexOf("e") != -1 || word.indexOf("i") != -1 || word.indexOf("o") != -1);
            if (containsVowel) {
                wordList.remove(i);
                i--;
            }
    ...
    public static ArrayList<String> parseWordsAndReverse(String sentence) {
        ...
        String updated = sentence;
        int idx = 0;
        while (updated.indexOf(" ") != -1) {
            idx = updated.indexOf(" ");
            String word = updated.substring(0, idx);
            returnList.add(0, word);
            updated = updated.substring(idx + 1);
        }
        returnList.add(0, updated);
        return returnList;
    }
